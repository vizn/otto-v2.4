...
esp_err_t WebSocketControlServer::ws_handler(httpd_req_t *req) {
    ...
    // ESP-IDF v6.0.2+ 中 WebSocket 握手由 httpd 内部完成，不会调用本 handler 的 GET 分支，
    // 因此在这里统一登记客户端（重复登记会被 AddClient 去重）
...
static void ws_broadcast_send_job(void* arg) {
    ...
    ESP_LOGI("WSControl", "BroadcastMessage: send fd=%d len=%d ret=%d", job->fd, (int)job->len, ret);
    if (ret != ESP_OK) {
...
void WebSocketControlServer::BroadcastMessage(const std::string& message) {
    ESP_LOGI(TAG, "BroadcastMessage: clients=%zu msg=%s", clients_.size(), message.c_str());
